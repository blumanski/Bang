...
class ModuleLoader
{
    ...
    private function lowLevelParameterValidation(string $module, string $controller, string $action)
    {
        ...
        if ($this->validateAppParamaters($module, $controller, $action) === false) {
            ...
            $this->ErrorLog->logError('App', 'App Parameter Test | Test failed -> | Module -> ' . htmlspecialchars($this->Route['module'], ENT_QUOTES, 'UTF-8') . ' | Controller  -> ' . htmlspecialchars($this->Route['controller'], ENT_QUOTES, 'UTF-8') . ' | Action -> ' . htmlspecialchars($this->Route['action'], ENT_QUOTES, 'UTF-8'), __FILE__ . ' -> Line ' . __LINE__ . ' -> ' . __METHOD__);
    ...
    public function executeModule(string $module, string $controller, string $action, array $params = array(), bool $return = false)
    {
        ...
        if (file_exists($moduleDirectory)) {
            ...
            if (file_exists($moduleDirectory . 'controllers' . DIRECTORY_SEPARATOR . $controller . 'Controller.php')) {
                ...
                $this->ErrorLog->logError('App', 'Action Call | File ' . MOD . htmlspecialchars($module, ENT_QUOTES, 'UTF-8') . '/Controller/' . htmlspecialchars($controller, ENT_QUOTES, 'UTF-8') . 'Controller.php' . ' does not exist', __FILE__ . ' -> Line ' . __LINE__ . ' -> ' . __METHOD__ . ' -> Module ->  ' . htmlspecialchars($module, ENT_QUOTES, 'UTF-8') . ' | Controller -> ' . htmlspecialchars($controller, ENT_QUOTES, 'UTF-8') . ' | Action -> ' . htmlspecialchars($action, ENT_QUOTES, 'UTF-8'));
            ...
            $this->ErrorLog->logError('App', 'Module Loader -> File ' . MOD . htmlspecialchars($module, ENT_QUOTES, 'UTF-8') . ' does not exist', __FILE__ . ' -> Line ' . __LINE__ . ' -> ' . __METHOD__ . ' -> Module ->  ' . htmlspecialchars($module, ENT_QUOTES, 'UTF-8') . ' | Controller -> ' . htmlspecialchars($controller, ENT_QUOTES, 'UTF-8') . ' | Action -> ' . htmlspecialchars($action, ENT_QUOTES, 'UTF-8'));
    ...
    public function loadModuleControllerAction(string $controllerToLoad, string $actionMethod, array $params = array(), bool $return = false)
    {
        ...
        if (class_exists($controllerToLoad, true)) {
            ...
            if (method_exists($ctrl, $actionMethod)) {
                ...
                // add to error log
                $this->ErrorLog->logError('App', 'Action Call | Method ' . MOD . htmlspecialchars($controllerToLoad, ENT_QUOTES, 'UTF-8') . '/' . htmlspecialchars($actionMethod, ENT_QUOTES, 'UTF-8') . ' does not exist', __FILE__ . ' -> Line ' . __LINE__ . ' -> ' . __METHOD__ . ' -> Controller -> ' . htmlspecialchars($controllerToLoad, ENT_QUOTES, 'UTF-8') . ' | ActionMethod -> ' . htmlspecialchars($actionMethod, ENT_QUOTES, 'UTF-8'));
            }
        } else {
            
            // add to error log
            $this->ErrorLog->logError('App', 'Module Class Exists | Class ' . htmlspecialchars($controllerToLoad, ENT_QUOTES, 'UTF-8') . ' does not exist.', __FILE__ . ' -> Line ' . __LINE__ . ' -> ' . __METHOD__ . ' -> Controller -> ' . htmlspecialchars($controllerToLoad, ENT_QUOTES, 'UTF-8') . ' | ActionMethod -> ' . htmlspecialchars($actionMethod, ENT_QUOTES, 'UTF-8'));
        }
